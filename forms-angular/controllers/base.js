...
formsAngular.controller('BaseCtrl', ['$scope', '$routeParams', '$location', '$http', '$filter', '$data', '$locationParse', '$modal', '$window', 'urlService', '$state', '$stateParse', 'fileUpload', '$route',
  function ($scope, $routeParams, $location, $http, $filter, $data, $locationParse, $modal, $window, urlService, $state, $stateParse, fileUpload, $route) {
    var master = {};
    ...
    $scope.$watchCollection("filequeue", function(obj){
      if (obj)
        if (angular.isArray(obj.file) && !!!$scope.files){
          $scope.files = fileUpload.fieldData.file;
          $scope.$watchCollection("files", function (obj, oldobj){
            if ($scope[$scope.topLevelFormName]){
              if (!_.isEqual(obj, oldobj)){
                $scope[$scope.topLevelFormName].$pristine = false;
              }
            }
          });
        }
    ...
    $scope.generateNewUrl = function (modelName) {
      modelName = modelName || $scope.modelName;
      return urlService.buildUrl(modelName + "/new");
    };
    $scope.generateListUrl = function(modelName) {
      modelName = modelName || $scope.modelName;
      return urlService.buildUrl(modelName);
    };
...
//                                    throw new Error('select2 initSelection called without a value');
          ...
          if (mongooseOptions.form.name) {
            $scope.filequeue.file = $scope.record.file;
...
//        }
    ...
    $scope.indexOf = function (array, obj) {
    ...
    $scope.scrollTheList = function () {
      ...
      $http.get('/api/' + $scope.modelName + generateListQuery()).success(function (data) {
        if (angular.isArray(data)) {
          ...
          for (var i in $scope.recordList){
            $scope.recordList[i].__unique_name = $scope.getListData($scope.recordList[i], $scope.listSchema[0].name);
          }
        } else {
    ...
    if (urlService.modelPage.test($location.path()))
      $scope.readRecord();
    else
      if (urlService.newRecord.test($location.path()))
        $scope.buildSchema();
        ...
        if (data.success !== false) {
          ...
          if (options.redirect) {
            ...
            if ($state && $state.params && $state.params.model) {
              ...
              $location.path($scope.generateEditUrl(data));
              //                    reset?
            }
          }
        } else {
    ...
    $scope.new = function () {
      ...
      if (urlService.newRecord.test($location.path()))
        $scope.record = {};
      $location.path($scope.generateNewUrl());
    ...
    $scope.deleteRecord = function (model, id) {
      $http.delete('/api/' + model + '/' + id).success(function () {
        ...
        for (var key in $scope.recordList)
          if ($scope.recordList[key]._id === id)
            $scope.recordList.splice(key, 1);
        $location.path($scope.generateListUrl());
      });
    };
              ...
              if (typeof $scope.dataEventFunctions.onBeforeDelete === 'function') {
                $scope.dataEventFunctions.onBeforeDelete(master, function(err) {
                  if (err) {
                    ...
                    $scope.deleteRecord($scope.modelName, id);
                  }
                });
              } else {
                $scope.deleteRecord($scope.modelName, id);
